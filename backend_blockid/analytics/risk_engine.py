...
Only new_wallet and (scam program) apply to them. Other types use full rules.
Risk level: 0 flags -> LOW, 1-2 -> MEDIUM, 3+ -> HIGH.
Daemon Protocol handles suspicious distribution detection.
"""
...
def calculate_risk(metrics: dict[str, Any], wallet_type: str | None = None) -> dict[str, Any]:
    ...
    For cold_wallet and service_wallet: only new_wallet is applied
    (not low_activity, not inactive). For others, all flags apply.
    Expects metrics: wallet_age_days, tx_count, unique_programs.
